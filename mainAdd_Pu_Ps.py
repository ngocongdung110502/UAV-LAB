...
def fitness(c, log=False):
    ...
    for i in range(0, N):
        ...
        # R_u at time slot i
        # formula (20)
        r_u_i = log2(1 + (theta * s[i]*P_s) / ((H**2 + d_su2)**a2))
        r_u = r_u + 1 * z[i] * t * r_u_i

        _o = ((H**2 + d_su2)**a2 * (H**2 + d_du2)**a2)
        # formula (21)
        r_d_i1 = log2(1+(theta*(n_u*w_0*s[i]*P_s+P_u_bar*(H**2 + d_su2)**a2)) / _o)

        # Data transmission rate from UAV to d if it cached a part of f file
        r_d_i2 = log2(1 + theta * u[i]*P_u / (H**2 + d_du2)**a2)
        # R_d at time slot i
        r_d_i = r_d_i1
        # r_d_i = r_d_i1 + r_d_i2
    ...
    for i in range(0, N1-1):
        ...
        e_fly_i = P_0 * (t + k_1 * dis**2) + P_1 * sqrt(sqrt(t**4 + k_2**2 * dis**4) -
                                                        k_2 * dis**2) + k_3 * dis**3 / t**2 + z[i] * t * (P_b + u[i]*P_u)
        e_fly = e_fly + e_fly_i
...
def init_one_child():
    ...
    for _ in range(1, N1-1):
        ...
        e_fly_i = P_0 * (t + k_1 * dis**2) + P_1 * sqrt(sqrt(t**4 + k_2**2 * dis**4) -
                                                        k_2 * dis**2) + k_3 * dis**3 / t**2 + _z * t * (P_b + _u*P_u)
...
X3 = list(range(2*N+2, 3*N+2))  # [0-40]
X4 = list(range(3*N+2, 4*N+2)) # [0-40]
X5 = list(range(4*N+2, 5*N+2)) # [0-40]
...
line,  = ax.plot(x1, y1, marker="o", linewidth=2, markersize=6)
...
ax.scatter(q_I[0], q_I[1], color="green", s=50)
ax.text(q_I[0], q_I[1], "Start", color="green")
ax.scatter(q_F[0], q_F[1], color="yellow", s=50)
ax.text(q_F[0], q_F[1], "Finish", color="green")
...
x1 = np.array(population[0][:N1]) * X
y1 = np.array(population[0][N1:N1+N1]) * Y
plt.plot(x1, y1, marker="o", linewidth=2, markersize=6)
plt.scatter(w_s[0], w_s[1], color="red", s=50)
plt.text(w_s[0], w_s[1], "Source", color="blue")
plt.scatter(w_d[0], w_d[1], color="blue", s=50)
plt.text(w_d[0], w_d[1], "Destination", color="red")
plt.scatter(q_I[0], q_I[1], color="green", s=50)
plt.text(q_I[0], q_I[1], "Start", color="green")
plt.scatter(q_F[0], q_F[1], color="yellow", s=50)
plt.text(q_F[0], q_F[1], "Finish", color="green")
plt.show()
f = open("result.txt", "a+")
